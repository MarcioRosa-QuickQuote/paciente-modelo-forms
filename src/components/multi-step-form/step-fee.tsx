...
export default function StepFee({ feeAmount, onYes, onNo, theme }: Props) {
  return (
    <div className="flex flex-col items-center justify-center min-h-[100dvh] px-6 py-8">
      ...
      >
        <div className="flex items-center gap-2 rounded-full px-4 py-2" style={{ background: theme.accentLight }}>
          ...
          <span className="text-xs font-medium" style={{ color: theme.accent }}>Valor abatido</span>
        </div>
        <div className="flex items-center gap-2 rounded-full px-4 py-2" style={{ background: theme.accentLight }}>
          <svg className="w-4 h-4" style={{ color: theme.accent }} fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M2.166 4.999A11.954 11.954 0 0010 1.944 11.954 11.954 0 0017.834 5c.11.65.166 1.32.166 2.001 0 5.225-3.34 9.67-8 11.317C5.34 16.67 2 12.225 2 7c0-.682.057-1.35.166-2.001zm11.541 3.708a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
          </svg>
          <span className="text-xs font-medium" style={{ color: theme.accent }}>Seguro</span>
        </div>
      </motion.div>
      ...
      <motion.div
        ...
        className="w-full"
      >
        <YesNoButtons onYes={onYes} onNo={onNo} yesText="Sim, concordo!" noText="Não" theme={theme} />
      </motion.div>
    </div>
  );
}

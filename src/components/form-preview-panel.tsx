...
import WorkflowOptionGrid from './workflow-option-grid';
...
function PreviewPergunta({ step, theme, desktop }: { step: FormStep; theme: Theme; desktop: boolean }) {
  ...
  const elementsStep = decisionMode
    ? { ...step, elements: (step.elements || []).filter(element => element.type !== 'buttons') }
    : step;
  ...
  return (
    <div className={`flex flex-col items-center ${desktop ? 'px-6 py-8' : 'px-3 py-5'}`}>
      ...
      {!!elementsStep.elements?.length && <PreviewExtraElements step={elementsStep} theme={theme} desktop={desktop} />}

      {decisionMode ? (
        <WorkflowOptionGrid options={step.workflowOptions || []} theme={theme} compact={!desktop} className="w-full grid gap-2.5" />
      ) : !hasCustomButtons && (
        <div className="flex gap-2 w-full">
          <Btn gradient={theme.yesBtn} text={step.yesText || 'Sim'} />
          <Btn text={step.noText || 'Não'} outlined />
        ...
        <div className="p-3">
          ...
          {form.slug && (
            <a
              ...
              className="mt-3 flex items-center justify-center gap-2 w-full py-2 rounded-xl border border-gray-200 text-xs font-semibold text-gray-600 hover:bg-gray-50 hover:border-gray-300 transition-all"
            >
